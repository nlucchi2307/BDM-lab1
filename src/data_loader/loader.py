...
class Loader:
    ...
    def load(self, g: DataGenerator):
        ...
        self._batch(
            """
            UNWIND $rows AS row        // row = [authorId, paperId, role]
        ...
        self._batch(self._unwind("Review", ["decision"]), g.reviews)

        self._batch("""
        UNWIND $rows AS row
        MATCH (p:Paper{id:row[0]}),(r:Review{id:row[1]})
        MERGE (p)-[:HAS_REVIEW]->(r)
        """, g.paper_in_review)

        self._batch("""
        UNWIND $rows AS row     // row = [authorId, reviewId, decision]
        MATCH (a:Author{id:row[0]}),(r:Review{id:row[1]})
        MERGE (a)-[v:VOTE]->(r)
        SET   v.decision = row[2]
        """, g.review_votes)
